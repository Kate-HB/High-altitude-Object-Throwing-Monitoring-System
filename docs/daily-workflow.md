...
- DeepSORT未安装（当前用IOU tracker降级，接口一致可延后替换）
- 结果视频画质可优化
- 报警弹窗/声音未实现
...
- `models/best.onnx` 已导出（10.3 MB, opset=17, imgsz=960, FP32）
- GPU推理验证通过（onnxruntime-gpu 1.20.1 + CUDAExecutionProvider）
- `scripts/test_model.py` 默认模型已改为 best.onnx
- TODO: .pt vs .onnx 性能对比（7/1）
...
当满足以下全部条件时，可以跳过逐人确认，连续执行：
1. 用户明确说"全部完成"、"继续所有"、"一次性完成"
2. 前序角色PR已合并（gh pr list 确认）
3. 当天无跨模块接口变更

快速模式下流程：
```
加载上下文 → 切换分支 → 拉取dev → 执行任务 → 提交PR → 自动进入下一角色
```

但每个角色的PR创建后仍需罗龙飞最终审核。快速模式节省的是中间等待用户确认的时间，不跳过代码审查和PR创建。

## 13. 工作流优化记录

- 2026/6/26：新增用户确认点机制，防止PR改动导致返工
- 2026/6/29：石义焌算法模块完成集成，更新第5节为已完成状态
- 2026/6/30：新增快速模式（第12节），补充ONNX状态（第5.4节），更新测试数量基准
